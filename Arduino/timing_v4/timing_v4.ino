...
This version has in its inner loop the rows (SR) and 
outer the MUXs (culumns)
...
void readArray(byte col){
  ...
  int readPin;
    ...
    Serial.write((byte) ((readPin>>8) & 0x0F));
    Serial.write((byte) (readPin & 0x0F));
    // *(dataPtr + 2*(row*6)) = (byte) (readPin & 0x0F); 
    // *(dataPtr + ( 2*(row*6) ) + 1) = (byte) ((readPin>>8) & 0x0F);

    readPin = analogRead(A1) - baseData[(row*24) + (col*6) + 1]; 
    if (readPin<0) readPin = 0;
    if (MAPPING_ENABLED==1) readPin = (int) (  exp( -( EXP_COEFF/(readPin+1) ) )*1023  );
    // *(dataPtr + 2*((row*6) + 1)) = (byte) (readPin & 0x0F);
    // *(dataPtr + ( 2*((row*6) + 1) ) + 1) = (byte) ((readPin>>8) & 0x0F);
    Serial.write((byte) ((readPin>>8) & 0x0F));
    Serial.write((byte) (readPin & 0x0F));
    ...
    // *(dataPtr + 2*((row*6) + 2)) = (byte) (readPin & 0x0F);
    // *(dataPtr + ( 2*((row*6) + 2) ) + 1) = (byte) ((readPin>>8) & 0x0F);
    Serial.write((byte) ((readPin>>8) & 0x0F));
    Serial.write((byte) (readPin & 0x0F));
    ...
    if (readPin<0) readPin = 0;
    if (MAPPING_ENABLED) readPin = (int) (  exp( -( EXP_COEFF/(readPin+1) ) )*1023  );
    // *(dataPtr + 2*((row*6) + 3)) = (byte) (readPin & 0x0F);
    // *(dataPtr + ( 2*((row*6) + 3) ) + 1) = (byte) ((readPin>>8) & 0x0F);
    Serial.write((byte) ((readPin>>8) & 0x0F));
    Serial.write((byte) (readPin & 0x0F));
    ...
    if (readPin<0) readPin = 0;
    if (MAPPING_ENABLED) readPin = (int) (  exp( -( EXP_COEFF/(readPin+1) ) )*1023  );
    // *(dataPtr + 2*((row*6) + 4)) = (byte) (readPin & 0x0F); 
    // *(dataPtr + ( 2*((row*6) + 4) ) + 1) =(byte) ((readPin>>8) & 0x0F);
    Serial.write((byte) ((readPin>>8) & 0x0F));
    Serial.write((byte) (readPin & 0x0F));
    ...
    if (readPin<0) readPin = 0;
    if (MAPPING_ENABLED) readPin = (int) (  exp( -( EXP_COEFF/(readPin+1) ) )*1023  );
    // *(dataPtr + 2*((row*6) + 5)) = (byte) (readPin & 0x0F); 
    // *(dataPtr + ( 2*((row*6) + 5) ) + 1) =(byte) ((readPin>>8) & 0x0F);
    Serial.write((byte) ((readPin>>8) & 0x0F));
    Serial.write((byte) (readPin & 0x0F));
