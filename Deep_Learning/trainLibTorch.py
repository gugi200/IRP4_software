...
PATH_TRAIN = Path("extended_train_alpha8soft_30_255")
PATH_TEST =  Path("extended_test_alpha8soft_30_255")
...
def create_dataset(path, batchsize, preprocess=None, mean=None, std=None, seed=None):
    '''
    input:
...
def train_step(model, metric, loss_fn, optimizer,
    ...
    if debug:
        print(f'Train loss: {train_loss:.4f} | Train acc: {train_acc*100:0.4f}% | Data lodaer step length: {len(data_loader)}')
...
def test_step(model, metric, loss_fn, data_loader, device, debug=False, wnb=True):
    ...
    with torch.inference_mode():
        ...
        if debug:
            print(f'Test loss: {test_loss:.4f}  |  Test acc: {test_acc*100:.4f}% | Data lodaer step length: {len(data_loader)}')
...
def visualize_preds(model, dataloader, class_names, batchsize):
    ...
    plt.title('Model predictions\nprocessed images: Exponential mapping (alpha=25), \nNormalization (mean=0.5, std=0.001)', 
              fontsize=15, pad=40, y=1)
    plt.axis(False)
    model = model.cpu()
    ...
    for i in range(9):
        ...
        ax.imshow(X_sample.squeeze().permute([1, 2, 0]), cmap='gray')
        ...
        if pred_label==truth_label:
            ax.set_title(title_text, fontsize=12, c='g');
        else:
            ax.set_title(title_text, fontsize=12, c='r');
        ax.axis(False)
...
def plot_decision_matrix(class_names, y_pred_tensor, targets, title):
