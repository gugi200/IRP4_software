                ...
                while (ser.is_open):
                    line = ser.read_until(
                            expected = stopByte1.to_bytes(1, 'big') + stopByte0.to_bytes(1, 'big') + \
                                            stopByte1.to_bytes(1, 'big') + stopByte0.to_bytes(1, 'big') ,
                            size = 1156
                        )

                    sensorValues = [int.from_bytes(line[x:x+2], 'big') for x in range(0, 1156, 2)]

                    array = np.array(sensorValues[:-2])

                    reshaped_array = array.reshape(96, 6)
    ...
    def makePrediction(self, data):
        if self.nonlinearT:
            ...
            preprocess = transforms.Compose([
                ...
                transforms.Resize(size=(224, 224)),
                transforms.ToTensor(),
                transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.001, 0.001, 0.001]),
            ])
